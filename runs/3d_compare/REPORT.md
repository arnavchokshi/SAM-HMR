...
`comparison/side_by_side.mp4` — 2570×720 @ 30 fps, 3.2 MB.

| Left panel | Right panel |
|---|---|
| `intermediates/frames/<frame>.jpg` (raw video, 896-cap) | `sam_body4d/rendered_frames/<frame>.jpg` (Body4D mesh overlay) |

PromptHMR-Vid only emits 3D-scene viewers (`world4d.{mcs,glb}`)
not per-frame 2D overlays; baking PHMR mesh overlays would require
running our own SMPL-X renderer over the per-frame fits in
`results.pkl` — see followup #3.

## 3. Validated unit-test coverage

```
tests/threed/  →  161 passed, 3 skipped, 1 warning  (0.83 s)
```
...
| `test_sidecar_promthmr_*` | 37 | mask sidecar + PHMR runner + project_joints |
...
| `test_compare_metrics.py` | 12 | jitter, MPJPE, foot-skating (NaN-safe) |
| `test_compare_render.py` | 10 | h-stack + letterbox + gutter |
| `test_compare_run.py` | 8 | align + auto-reduce + e2e driver |
| `test_orchestrator.py` | 15 | per-stage cmd builders + skip-flag composer |
...
metric table and a comparative summary across clips. The Stage D
followups (Procrustes alignment, PHMR mesh overlays, world-frame
foot-skating) will materially change the headline numbers and
should land before the cross-clip comparison is interpreted.
...
1. **Procrustes-aligned MPJPE.** Add a `align_procrustes(joints_a,
   ...
   MPJPE: <50 cm for COCO body joints.
...
3. **PromptHMR mesh overlays.** Run our own SMPL-X renderer (use
   `pyrender` or borrow PHMR's bundled one) over the per-frame fits
   in `results.pkl`, write `prompthmr/rendered_frames/<frame:08d>.jpg`,
   and pass to the orchestrator's `--prompthmr-frames-dir` flag for
   the side-by-side mp4.
