...
Generated 2026-04-18 from commit [`e33572d`](https://github.com/arnavchokshi/SAM-HMR/commit/e33572d).
...
- **Right** is generated by `threed/sidecar_body4d/render_overlay.py`
  ...
  stage between `body4d` and `compare`. Re-uses SAM-Body4D's own
  ``vertices_to_trimesh`` convention: PLY vertices are pre-flipped
  ``(pred_verts + cam_t) * [1, -1, -1]`` so the scene only adds a
  per-dancer translation (`body4d_dancer_world_pos`) before adding
  to a single multi-dancer pyrender scene with the shared per-frame
  focal. Closes followup #6.
...
```
tests/threed/  →  239 passed, 3 skipped, 1 warning  (0.9 s)
```
...
| `test_body4d_render_overlay.py` | 19 | dancer-id discovery, focal JSON parse, body4d_dancer_world_pos (incl. anti-double-flip pin), flip_yz_verts |
...
6. ~~**SAM-Body4D mesh overlays on real video.**~~ **Done
   (commits `77a700e`, `37a01f8`, `e33572d`):**
   ...
   single-scene-per-frame design re-uses SAM-Body4D's `cam_t.x`
   sign convention for camera world position and respects that the
   PLY vertices are already pre-flipped by upstream's
   `vertices_to_trimesh`, so we don't double-flip. Wall on adiTest:
   33 s. Closes the "Body4D panel is clean background" issue.

## 7. Reproducibility receipts

- Repo HEAD: [`e33572d`](https://github.com/arnavchokshi/SAM-HMR/commit/e33572d) on `main`
...
- Tests: `pytest tests/threed/ -q` → 239 passed, 3 skipped, 1 warning
