...
| 2026-04-18 | Followup #6 — SAM-Body4D mesh overlay on real video (closes the right-panel clean-background gap) | OK — see preceding row for full description. (kept here as an anchor for cross-references; metrics already merged.) |
| 2026-04-18 | Followup #2 — world-frame foot-skating | OK — `foot_skating_world_frame()` in `metrics.py` calibrates a per-dancer floor (min Y per dancer's foot joint) and reports planted-foot velocity (height < `threshold_m` above floor). Wired into `run_compare.py` as `--prompthmr-world-joints`/`--world-foot-idx`/`--world-foot-threshold` and into orchestrator (`scripts/run_3d_compare.py`) so the field `foot_skating_phmr_world_m_per_frame` lands in `metrics.json` whenever `prompthmr/joints_world.npy` exists. 8 unit tests for the metric (static, motion, lifted feet, per-dancer calibration, NaN handling, axis swaps), 2 wiring tests in `test_compare_run.py`, 2 wiring tests in `test_orchestrator.py`. Cross-clip world FS values 0.005–0.031 m/frame — markedly lower than the cam-frame metric (0.02–0.06) which used to sit near zero only because the camera is mounted above the floor. Commit `915fc5f` (metric), `e8af9bb` (wiring). |
| 2026-04-18 | Followup #4 — 2D reprojection vs ViTPose (PHMR side) | OK — `threed/sidecar_promthmr/reproject_vs_vitpose.py` loads `results.pkl` (joblib) + `joints_coco17_cam.npy` and reprojects PHMR's 3D COCO-17 joints through PHMR's per-frame intrinsics (`img_focal`, `img_center`), then compares against the bundled ViTPose 17-keypoint detections after masking out keypoints with confidence < `--vitpose-conf-threshold` (default 0.3). Helpers `reproject_3d_to_2d` and `mpjpe_2d` added to `metrics.py` (NaN-safe, pinhole, 11 new unit tests). End-to-end module covered by 7 tests (padding, slot ordering, missing tids, low-conf masking, parent-dir creation). Body4D-vs-ViTPose deferred — PHMR's intrinsics live in resized image space (504×896) while Body4D operates on native 1280×720 with no exposed intrinsics, so the two pipelines need a shared reference frame before they can be cross-compared. Cross-clip PHMR-vs-ViTPose pixel MPJPE: 9.6 (easyTest) to 14.5 (loveTest) px — except 2pplTest at 43.3 px, where the dancers occupy ~5% of the frame and ViTPose is unreliable. Commits `f2b1c8e`, `c06c2a4`. |
| 2026-04-18 | 5-clip extension batch — final results | OK — all 6 clips (adiTest + 5 extension) processed end-to-end on Lambda A100 with `--max-frames 188 --batch-size 32 --disable-completion`. Per-clip wall (body4d only): 2pplTest 6 m, easyTest 15 m, gymTest 15 m, BigTest 17 m, loveTest 28 m (15 dancers). Headline metrics: PA-MPJPE 0.18-1.20 m (raw 4.5-14.5 m, useless without alignment); PHMR jitter 0.014-0.049 m/frame, Body4D jitter 0.011-0.076 m/frame; PHMR world-frame foot-skating 0.005-0.031 m/frame. **Significant outlier:** gymTest PA-MPJPE = 1.20 m vs the rest's 0.18-0.74 m — likely camera dolly + occluded gym equipment causes per-dancer scale drift between the two pipelines. **Followup #5** (HTML report): `scripts/build_html_report.py` writes a self-contained `runs/3d_compare/report.html` (10.7 kB) with embedded `<video>` per clip + cross-clip summary table. 11 new unit tests (286 total). All artifacts synced to `runs/3d_compare/<clip>/comparison/{metrics.json,reproj_metrics.json,side_by_side.mp4}`. Commits `9038902` (HTML report). |
