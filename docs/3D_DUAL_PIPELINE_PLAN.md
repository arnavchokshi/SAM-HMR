...
| 2026-04-18 | Followup #6 — SAM-Body4D mesh overlay on real video (closes the right-panel clean-background gap) | OK — `threed/sidecar_body4d/render_overlay.py` reads the cached `mesh_4d_individual/<id>/<frame>.ply` + `focal_4d_individual/<id>/<frame>.json` tree and alpha-composites each dancer's MHR mesh onto the original RGB frame via headless `pyrender` (single-scene-per-frame, alpha=0.65, shares HSV-wheel palette + composite helpers with PHMR sidecar). Per-dancer translation `body4d_dancer_world_pos(cam_t) = [2*cam_t.x, -cam_t.y, -cam_t.z]` replicates SAM-Body4D's `Renderer` convention (camera world position = `[-cam_t.x, cam_t.y, cam_t.z]` plus a 180° X-rotation already pre-baked into the PLY by upstream's `vertices_to_trimesh`). 19 unit tests pin dancer-id discovery, JSON parsing, the translation sign convention (regression pin against the double-flip bug), and `flip_yz_verts`. Wired into `scripts/run_3d_compare.py` as the new `body4d_render` stage between `body4d` and `compare` (runs in `body4d` env via `conda run`); `--skip-body4d-render` flag added; `--skip-body4d` implies it. The final `render` step auto-detects `sam_body4d/rendered_frames_overlay/` on disk and prefers it over upstream's clean-background `rendered_frames/`. **Bug fixed during box bring-up:** initial draft applied `flip_yz_verts` *inside* `_build_pyrender_scene`, double-flipping the already-pre-flipped PLYs and pushing meshes behind `pyrender`'s znear=0.1 plane → blank overlay; root-caused by inspecting `vertices_to_trimesh` in upstream and dumping raw render alpha/depth on the box; fix removes the redundant flip and updates docstrings + adds anti-double-flip regression test. adiTest re-render: `body4d_render` 33 s wall, compare+render 4 s wall, `side_by_side.mp4` now 10.4 MB with both panels showing colored mesh overlays on the real input frames. Total tests: 239 passed (was 220). Commits `77a700e`, `37a01f8`, `e33572d`. |
